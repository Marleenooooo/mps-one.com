...
export class BridgeClient {
    ...
    async getFunnelData(token) {
        try {
            ...
            if (!res.ok)
                throw new Error(`funnel ${res.status}`);
            return await res.json();
        }
        catch {
            return {
    ...
    async getSupplierPerformance(vendorId, token) {
        try {
            ...
            if (!res.ok)
                throw new Error(`supplier ${res.status}`);
            return await res.json();
        }
        catch {
            return {
    ...
    async getCohortData(token) {
        try {
            ...
            if (!res.ok)
                throw new Error(`cohorts ${res.status}`);
            return await res.json();
        }
        catch {
            return [
    ...
    async getAnomalies(token) {
        try {
            ...
            if (!res.ok)
                throw new Error(`anomalies ${res.status}`);
            return await res.json();
        }
        catch {
            return [
    ...
    subscribeToAnalyticsEvents(topics = [], onEvent) {
